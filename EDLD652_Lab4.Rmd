...
HighlightFlippers <- penguins %>% filter(flipper_length_mm > 225, bill_length_mm > 55)
FlipperxBillGood <- ggplot(penguins, aes(x = bill_length_mm, y = flipper_length_mm)) + geom_point(size = 3, alpha = 0.7) + labs(title = "Scatterplot of Bill Length vs. Flipper Length", x = "Bill Length", y = "Flipper Length") + geom_point(data = HighlightFlippers,
size = 3,
color = "red") + geom_text_repel(data = HighlightFlippers,
...
penguinscount
penguinscount <- as.data.frame(penguinscount)
CountxSpecies <- ggplot(penguinscount, aes(x = species, y = n, fill = species)) +
  geom_bar(stat="identity") + theme_minimal() + labs(title = "Penguin Count by Species", x = "Species", y = "Cost") +  scale_fill_manual(name='Species', labels=c('Adelie', 'Chinstrap', 'Gentoo'),
