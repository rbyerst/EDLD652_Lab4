...
```
**Reflection** 
Using the "scale_manual_color" approach seems to allow more control here, as you could be very specific with which color you want to ouse for the highlighting. 
...
```{r}
FlipperxBillBad <- ggplot(penguins, aes(x = flipper_length_mm, y = bill_length_mm, color = bill_length_mm)) + geom_point(size = 3, alpha = 0.7) + labs(title = "Scatterplot of Flipper Length vs. Bill Length by Species", x = "Flipper Length", y = "Bill Length") + scale_color_manual("rainbow") + theme_minimal()
FlipperxBillBad
```
